...
export default {
    ...
    data() {
        return {
            ...
            // table
            tableTitleData: [
                this.$t('supplyReport.date'),
                this.$t('supplyReport.souceName'),
                this.$t('supplyReport.cangku'),
                this.$t('supplyReport.unit'),
                this.$t('supplyReport.type'),
                this.$t('supplyReport.zhangmian'),
                this.$t('supplyReport.shishiCount'),
                this.$t('supplyReport.sunyiCount'),
                this.$t('supplyReport.sunyiAmount'),
            ],
            requestData: {
                token: "",
                page_num: 0,
    ...
    methods: {
        ...
        getProfitLossReport() {
            ...
            this.request.getProfitLossReport(this.requestData, function(res) {
                if (res.code == "1") {
        ...
        download(email, isdefault) {
            let that = this;
            this.requestData.report_type="2";
        ...
        storehouseList() {
            ...
            this.request.storehouseList(ListData, function(res) {
                if (res.code == "1") {
                    ...
                    req_data.map((value, index, req_data) => {
                        labelArray.push(value.storehouse_name);
        ...
        orgCheckChange(data, checked, indeterminate) {
            ...
            } else if (indexs >= 0 &&that.selectOrg.orgsid.length === 1 &&!checked) {
                // 再次直接进行赋值为空操作
                that.selectOrg.orgsid = [];
