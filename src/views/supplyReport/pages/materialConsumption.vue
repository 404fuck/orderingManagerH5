    ...
    <review-container>
        <view-content slot="content">
            <div slot="cont-label" class="row">
                <div class="content-span col-50" style="position:relative;top:20px">
                    <span>{{$t('supplyReport.total')}}</span>
                    <span>{{$t('supplyReport.xiaohaoAmount')}}：{{getSum}}</span>
                </div>
                <div class="col-50" style="text-align:right">
                    <div class="header-download" @click="downloadCLick" style="display:inline-block;margin-right:15px;">{{$t('supplyReport.download')}}</div>
                    <div class="content-filtrate" @click="selectBtn" style="display:inline;font-size:14px;font-weight:bold">{{$t('supplyReport.select')}}</div>
                </div>
            </div>
                ...
                <table v-loading="dataLoading">
                    ...
                    <tbody>
                        <tr v-for="(item,index) in filltableData" :key="index">
                            ...
                            <td>{{item.expend_count|count}}</td>
                            <td>{{item.expend_amount|price}}</td>
                        </tr>
                        <tr v-if="filltableData==null||filltableData.length==0" > 
                            <td colspan="6"><span>{{$t('noData')}}</span></td>
                        </tr>
                    </tbody>
                </table>
            ...
            >
                <filtrate-view>
                    <filtrate-content slot="filtrate-content">
                        ...
                        ></filtrate-input>
                        <filtrate-disable  v-model="filtratedisableData"  :label="$t('supplyReport.type')+'：'" @click.native="selectDialog=true"></filtrate-disable>
                        <filtrate-date-range
                            v-model="filtraterangeDate"
                            :label="$t('supplyReport.date')+'：'"
                            format="yyyy-MM-dd"
                        ></filtrate-date-range>
                    </filtrate-content>
            ...
            >
                <email-view>
                    <email-content slot="email-content">
                        ...
                        <email-checkbox v-model="emailcheckboxData"></email-checkbox>
                    </email-content>
                    <email-footer slot="email-footer">
                        <div slot="cancel" @click="cancelClick">{{$t('cancel')}}</div>
                        <div slot="send" @click="sendClick">{{$t('send')}}</div>
                    </email-footer>
                </email-view>
            </el-dialog> -->
            <!-- 类别选择弹出层 -->
            ...
            >
                <selectType-view>
                    ...
                    <selectType-footer slot="selectType-footer">
                        <div slot="selectSure" @click="selectSureClick">{{$t('supplyReport.confirm')}}</div>
                    </selectType-footer>
                </selectType-view>
            </el-dialog>
        </view-model>
...
export default {
    ...
    components: {
        reviewContainer:resolve => require(['../../components/reviewContainer/reviewContainer'],resolve),
        viewContent:resolve => require(['../../components/reviewContainer/pageView/viewContent'],resolve),
        viewModel:resolve => require(['../../components/reviewContainer/pageView/viewModel'],resolve),
        filtrateView:resolve => require(['../../components/filtrateDrawer/filtrateView/filtrateView'],resolve),
        filtrateContent:resolve => require(['../../components/filtrateDrawer/filtrateView/filtratePage/filtrateContent'],resolve),
        filtrateFooter:resolve => require(['../../components/filtrateDrawer/filtrateView/filtratePage/filtrateFooter'],resolve),
        filtrateSelect:resolve => require(['../../components/filtrateDrawer/filtrateComponents/filtrateSelect'],resolve),
        filtrateInput:resolve => require(['../../components/filtrateDrawer/filtrateComponents/filtrateInput'],resolve),
        filtrateDisable:resolve => require(['../../components/filtrateDrawer/filtrateComponents/filtrateDisable'],resolve),
        filtrateDateRange:resolve => require(['../../components/filtrateDrawer/filtrateComponents/filtrateDateRange'],resolve),
        emailView:resolve => require(['../../components/emailPop/emailView/emailView'],resolve),
        emailContent:resolve => require(['../../components/emailPop/emailView/emailPage/emailContent'],resolve),
        emailFooter:resolve => require(['../../components/emailPop/emailView/emailPage/emailFooter'],resolve),
        emailCheckbox:resolve => require(['../../components/emailPop/emailComponents/emailCheckbox'],resolve),
        emailInput:resolve => require(['../../components/emailPop/emailComponents/emailInput'],resolve),
        selectTypeView:resolve => require(['../../components/selectTypePop/selectTypeView/selectTypeView'],resolve),
        selectTypeHeader:resolve => require(['../../components/selectTypePop/selectTypeView/selectTypePage/selectTypeHeader'],resolve),
        selectTypeContent:resolve => require(['../../components/selectTypePop/selectTypeView/selectTypePage/selectTypeContent'],resolve),
        selectTypeFooter:resolve => require(['../../components/selectTypePop/selectTypeView/selectTypePage/selectTypeFooter'],resolve),
    ...
    data() {
        return {
            ...
            tableTitleData: [
                ...
                this.$t('supplyReport.xiaohaoCount'),
                this.$t('supplyReport.xiaohaoAmount'),
            ],
            requestData: {
                token: "",
                page_num: 0,
    ...
    methods: {
        ...
        getExpendInfo() {
            ...
            this.request.getExpendInfo(this.requestData, function(res) {
                if (res.code == "1") {
        ...
        getExpendSum(){
            ...
            this.requestData.begin_date = this.beginDate; //传入开始时间
            this.requestData.end_date = this.endDate; //传入结束时间
            this.request.getExpendSum(this.requestData, function(res) {
                if (res.code == "1") {
                    that.getSum = res.data
                }
            })
        },
        //分页初始化
        ...
        download(email, isdefault) {
            let that = this;
            this.requestData.report_type="3";
        ...
        sureClick() {
            ...
            if (fselData === null&& finpData === "" && fdisData==="" && frangData.length===0) {
